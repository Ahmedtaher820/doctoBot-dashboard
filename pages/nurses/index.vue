...
if(nursesState.value.length === 0){
  ...
  $fetch(`${config.public.apiUrl}/nurses`, {
    params: {
      page: page.value,
    },
    headers: {
...
<template>
  <div>
    <SystemTable @add="addDoctor">
      ...
      <tr v-for="doctor in nursesState" :key="doctor._id">
        <SystemTableThGrid>
          <div class="flex items-center">
            <div class="h-11 w-11 flex-shrink-0">
              <img class="h-11 w-11 rounded-full object-contain" :src="doctor?.image" alt="" />
            </div>
            <div class="ml-4">
        ...
        <SystemTableThGrid >{{ doctor.shift }}</SystemTableThGrid>
        <SystemTableThGrid
