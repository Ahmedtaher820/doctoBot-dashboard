...
const { deleteNurses, getAllNurses } = nursesStore()
...
const deleteNursesConfirm = () => {
  deleteNurses(selectedNurses.value?._id).then(() => {
    nursesState.value = nursesState.value.filter((item) => item._id !== selectedNurses.value?._id)
  }).then(() => {
...
const updated = () => {
  ...
  getAllNurses().then((res) => {
    nursesState.value = res?.data
...
<template>
  <div>
    <AppConfirmModal :show="showConfirmModal" :processing="processing" @close="showConfirmModal = false"
      @cancel="showConfirmModal = false" @delete="deleteNursesConfirm" cateInfo="Nurses info" />
    <NursesModifyNursesModal :nurses="selectedNurses" @updated="updated" :show="showModal" @close="showModal = false" />
    <over-lay-loader v-if="processing" />
    <SystemTable @add="addDoctor" class="shadow-md shadow-gray-200">
      <template #grid-header>
        <tr>
          <SystemTableThGrid>Name</SystemTableThGrid>
      ...
      <tr v-for="nurses in nursesState" :key="nurses._id">
        <SystemTableThGrid>
          <div class="flex items-center">
            <div class="h-11 w-11 flex-shrink-0">
              <img v-if="nurses?.image !== '{}'" class="h-11 w-11 rounded-full object-contain" loading="lazy" :src="nurses?.image" alt="" />
        ...
        <SystemTableThGrid>
          <span
            ...
            :class="[nurses.isAvailable ? 'text-green-700' : 'text-red-600']">{{ nurses.isAvailable ? 'Active' :
              'noActive' }}</span>
        </SystemTableThGrid>
